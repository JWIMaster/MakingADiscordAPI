...
public var token: String? {
    get {
        return StringCache.load(for: "discordToken")
    }
    set {
        if let value = newValue {
            StringCache.store(value, for: "discordToken")
        } else {
            let url = StringCache.filePath(forKey: "discordToken")
            try? FileManager.default.removeItem(at: url)
            StringCache.memoryCache.removeObject(forKey: "discordToken" as NSString)
        }
    }
}
